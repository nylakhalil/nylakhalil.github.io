  ...
  getIconCol(item) {
    return (
      <Col className="text-center" key={item.name}>
        <span
          className="cursor-pointer icon-circle"
  ...
  getDescription(description) {
    return (
      <blockquote className="mr-5 blockquote pl-5 pt-4">
        <p className="text-muted">{description["content"]}</p>
        <footer className="blockquote-footer text-center">
          <small className="text-muted mr-5">
            <a href={description["link"]}>
              <cite title="Source Link">{description["source"]}</cite>
            </a>
          </small>
        </footer>
      </blockquote>
      ...
      <div className="d-block p-5">
        ...
        <Row className="align-self-start" noGutters={true}>
          <Col className="bg-light">
            {this.getWorldCloud(
              this.state.content[title]["words"],
              this.state.options
            )}
          </Col>
